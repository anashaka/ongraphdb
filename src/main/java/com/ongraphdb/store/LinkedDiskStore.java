...
public class LinkedDiskStore implements DiskStore{
	...
	public LinkedDiskStore(String dataFileName, int dataMappedMemorySize, int initialFileSize) {
	...
	private RecordHeader readRecordHeader(int pos) throws IOException {
		if (isMapped(pos + RecordHeader.HEADER_BYTES)) {
			return readRecordHeaderFromMappedBuffer(pos);
		} else {
			return readRecordHeaderFromChannel(pos);
		}
	}
